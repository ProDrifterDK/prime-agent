...
- Fixed `openai-codex` models being invisible to `find_models` and RLM subagents because model discovery sent the Prime Agent version as the Codex `client_version` and cached an empty catalog as a successful result ([#702](https://github.com/PrimeIntellect-ai/prime-agent/issues/702)).
